...
class ResidualDynamicsDataset(Dataset):
    def __init__(self, data_store: Dict,device='cpu'):
        ...
        for i in range(len(times)-1):
            ...
            if all(k in s for k in required) and all(k in sn for k in required):
                examples.append((t, tn))
        self.examples_list[0] = examples
        self.data_store_dict[0] = data_store

    def add_more_data(self, new_data_store: Dict, index):
        times = sorted(new_data_store.keys())
        examples = []
        for i in range(len(times)-1):
            t, tn = times[i], times[i+1]
            # require dt > 0
            ...
            # store pairs if both entries are valid
            s = new_data_store[t]
            sn = new_data_store[tn]
            # basic validation
            required = ["position", "rotation_matrix", "velocity", "action", "lin_acc"]
            if all(k in s for k in required) and all(k in sn for k in required):
                examples.append((t, tn))
        self.examples_list[index] = examples
        self.data_store_dict[index] = new_data_store
    ...
    def __len__(self):
        total = 0
        for ii in range(len(self.examples_list)):
            total += len(self.examples_list[ii])
        return total

    def _read(self, t, index):
        data_store = self.data_store_dict[index]
        e = data_store[t]
        # position
    ...
    def __getitem__(self, idx):
        num_index = len(self.examples_list)
        rand_num = random.randint(0, num_index - 1)
        examples = self.examples_list[rand_num]
        total_num = len(examples)
        rand_index = random.randint(0, total_num - 2)
        t, tn = examples[rand_index]
        dt = float(tn - t)
        
        p, v, R, action, _ = self._read(t, rand_num)
        # p_n, v_n, R_n, _ = None, None, None, None
        _, _, _, _, lin_acc = self._read(tn, rand_num)  #lin_acc is mass normalized acc - gravity already in world frame
        # convert to torch
...
# Full training loop
# ----------------------------
def train(data_store_path_list: list,
    ...
    # load pickle
    for jj,data_store_path in enumerate(data_store_path_list):
        with open(data_store_path, "rb") as f:
            if jj == 0:
                data_store = pickle.load(f)
                ds = ResidualDynamicsDataset(data_store)
            else:
                curr_data_store = pickle.load(f)
                ds.add_more_data(curr_data_store, jj)
...
if __name__ == "__main__":
    ...
    full_load_path_list = []
    for curr_file in file_num:
        full_load_path_list.append(os.path.join(load_directory, "data_collected_" + str(curr_file) + ".pkl"))
    full_save_path = os.path.join(load_directory, "model_" + str(file_num[-1]+ 1) + ".pt")

    train(full_load_path_list, model_save_path=full_save_path, epochs=args.epochs, batch_size=args.batch, drone_mass = 0.234)
    # main()

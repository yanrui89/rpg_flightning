...
class ResidualDynamicsDataset(Dataset):
    def __init__(self, data_store: Dict,device='cpu'):
        ...
        """
        self.device = device
        self.combined_data_store = {}
        self.combined_examples = []  # list of (t, t_next)
        counter = 0
        for curr_data_store in data_store:
            # Sort timestamps
            times = sorted(curr_data_store.keys())
            for i in range(len(times)-1):
                t, tn = times[i], times[i+1]
                #store into new dictionary
                self.combined_data_store[counter] = curr_data_store[t]
                counter += 1
                # require dt > 0
                dt = tn - t
                if dt <= 0:
                ...
                if all(k in s for k in required) and all(k in sn for k in required):
                    if curr_data_store[t]['time'] == -1 or curr_data_store[tn]['time'] == -1:
                        if i == (len(times)-2):
                            self.combined_data_store[counter] = curr_data_store[tn]
                            counter += 1
                        continue
                    else:
                        self.combined_examples.append((counter -1, counter))
    ...
    def __getitem__(self, idx):
        t, tn = self.combined_examples[idx]
        dt = float(tn - t)
